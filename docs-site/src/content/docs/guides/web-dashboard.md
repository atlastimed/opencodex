...
| **Models** | Toggle native GPT and routed models, set provider allowlists and context caps, choose v1/base/v2, and configure the v2 thread limit. Configured providers stay visible as zero-model groups when discovery is off or returns no rows. When live discovery is enabled and the catalog fetch fails, configured and custom models remain listed; the group shows the failure reason and a link to Provider Settings (`#providers`), where **Discover models from provider** lives. |
