...
| **Models** | 开关原生 GPT 与路由模型，配置 provider allowlist、上下文上限、v1/base/v2 以及 v2 thread 数量。实时发现已启用且目录获取失败时，已配置和自定义模型仍会列出；分组会显示失败原因，并提供指向提供方设置（`#providers`）的链接，其中包含「从提供方发现模型」。 |
