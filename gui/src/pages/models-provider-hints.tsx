...
export function DiscoveryFailedHint({
  discovery,
  ...
  return (
    <div className="row muted text-label leading-body" role="status" style={{ alignItems: "flex-start", gap: 8, padding: "6px 0" }}>
      ...
      <span>
        {showFailureBadge && <><span className="badge badge-amber">{t("models.discoveryFailedBadge")}</span>{" "}</>}
        {`${discoveryFailureLabel(t, discovery)} ${t("models.discoveryFailedGuidance", { setting: t("pws.liveModels") })} `}
...
export function EmptyProviderHint({
  ...
  if (failed) {
    return <DiscoveryFailedHint discovery={failed} showFailureBadge={showFailureBadge} />;
  }
  return (
    <div className="row muted text-label leading-body" role="status" style={{ alignItems: "flex-start", gap: 8, padding: "6px 0" }}>
      <IconInfo width={15} height={15} aria-hidden="true" style={{ flexShrink: 0, marginTop: 2 }} />
